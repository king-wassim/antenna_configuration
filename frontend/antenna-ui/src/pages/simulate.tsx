...
async function saveSimulationToHistory(
...
export default function Simulate() {
  ...
  const handleSimulate = () => {
    ...
    simulateMutation.mutate(
      ...
      {
        onSuccess: (result) => {
          void saveSimulationToHistory(config, theta0, result.metrics).catch((err: unknown) => {
            toast({
        ...
        onError: (err: unknown) => {
          toast({
            ...
            description: getErrorMessage(err, "An unknown error occurred"),
            variant: "destructive",
          });
        },
      }
