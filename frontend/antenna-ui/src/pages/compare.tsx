...
function toAccuracyPercent(globalError: number): number {
  return Math.max(0, 100 - globalError);
}
...
async function savePredictionRecord(
  ...
  compResult: {
    reference: { metrics: { hpbw: number; mainLobeGain: number; sideLobeLevel: number } };
    predicted: { metrics: { hpbw: number; mainLobeGain: number; sideLobeLevel: number } };
    errors: { globalError: number };
  }
): Promise<void> {
  const response = await fetch("/api/predictions", {
    ...
    body: JSON.stringify({
      ...
      referenceConfig: snapshot.config,
      predictedConfig,
      referenceMetrics: snapshot.metrics,
      predictedMetrics: compResult.predicted.metrics,
      globalError: compResult.errors.globalError,
    }),
  });
  ...
  if (!response.ok) {
    ...
    try {
      ...
      // Keep the HTTP status message when the body cannot be parsed.
    }
      ...
      { data: { image: snapshot.patternImageBlob as Blob } },
      {
          ...
          compareMutation.mutate(
            ...
            },
            {
              onSuccess: (compResult) => {
                ...
                void savePredictionRecord(snapshot, predictedCfg, compResult)
                  .then(() => {
                    ...
                    toast({
                      ...
                      description: "This simulation has been stored automatically.",
                    });
                  })
                  .catch((err: unknown) => {
                    toast({
                      title: "History save failed",
                      description: getErrorMessage(err, "Simulation completed but could not be saved"),
                      variant: "destructive",
                    });
                  });
              },
              onError: (err: unknown) => {
                toast({
                  ...
                  description: getErrorMessage(err, "Unknown error during comparison"),
                  variant: "destructive",
                });
              },
            }
        ...
        onError: (err: unknown) => {
          toast({
            ...
            description: getErrorMessage(err, "ML model could not process the image"),
            variant: "destructive",
          });
        },
      }
          ...
          {compResult && (
            <div className="rounded-xl border border-border p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  { label: "Accuracy", val: toAccuracyPercent(compResult.errors.globalError).toFixed(2), unit: "%" },
                  { label: "Loss", val: compResult.errors.globalError.toFixed(4), unit: "" },
